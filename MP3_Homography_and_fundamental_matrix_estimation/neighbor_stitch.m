function output = neighbor_stitch(from, to,base)
    % overlapping detecting
    mask_ovlp = double((sum(from,3) & sum(to,3)) >0);
    mask_from = double(sum(from,3) > 0);
    ...
    mask = double(mask_from & mask_trans);   % from =1, to = 0;
