...
%
% Usage:  [cim, x, y] = harris(im, sigma, thresh, radius, disp)
%
% Arguments:   
...
%            x      - row coordinates of corner points.
%            y      - column coordinates of corner points.
%
...
function [cim, x, y] = harris(im, sigma, thresh, radius)
...
%   cim = (Ix2.*Iy2 - Ixy.^2) - k*(Ix2 + Iy2).^2; 
	...
	[x,y] = find(cim);                  % Find row,col coords.
	
% 	if nargin==6 & false      % overlay corners on original image
% 	    figure, imagesc(im), axis image, colormap(gray), hold on
% 	    plot(y,x,'ys'), title('corners detected');
%         fig = gcf;
%         fig.PaperPositionMode = 'auto';
%         capture = ['out/', sprintf('%s-harris',outname)];
